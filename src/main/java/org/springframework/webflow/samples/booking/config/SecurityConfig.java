...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http
			.formLogin()
				...
				.defaultSuccessUrl("/events/search")
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.inMemoryAuthentication()
			...
			User keith = new User("keith", "melbourne");
			User erwin = new User("erwin", "leuven");
			User jeremy = new User("erwin", "atlanta");
			User scott = new User("scott", "rochester");
			userRepo.deleteAll();
			userRepo.save(keith);
			userRepo.save(jeremy);
			userRepo.save(erwin);
			userRepo.save(scott);
			ctx.close();
	}
